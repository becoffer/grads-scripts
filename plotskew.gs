* Script to plot Skew-T diagram from cm1 GrADS-format output
* NOTE:  see many user settings below, near top of plotskew function
* Authors:  George H. Bryan, NCAR/MMM (cm1-related stuff)
*           Robert Hart, FSU (skew-T plotting script)
...
"define el=log(((1.0e-20+qv)/0.622)*prs/100.0/(1.0+((1.0e-20+qv)/0.622)))"
...
"define tmp=t-273.15"
***"define dew=td-273.15"

* Don't allow depoint depression to exceed 75 C  (GHB: 090303)
ddfix=75.0
"define dd=t-td"
"define td1=const(maskout(td,"ddfix"-dd),0.0,-u)"
"define td2=const(maskout(t-const(maskout(dd,"ddfix"-dd),"ddfix",-u),dd-"ddfix"),0.0,-u)"
"define dew=td1+td2-273.15"

"define spd=1.94*( mag(((uinterp+"umove")),((vinterp+"vmove"))) )"
"define dir=atan2(uinterp+"umove",vinterp+"vmove")*57.2957795+180.0"
...
*---------------
*  write out ctl file for skew.dat
*---------------

res=write('skew.ctl','DSET  ^skew.dat')
res=write('skew.ctl','UNDEF  -99999999.')
res=write('skew.ctl','TITLE CLT skewt scratch file')
res=write('skew.ctl','xdef 1 linear 1 1')
res=write('skew.ctl','ydef 1 linear 1 1')
res=write('skew.ctl','zdef 'zsize' levels  ')
n=1
while (n <= zsize)
   "set z "n
   "d "pressurename
   pval=subwrd(result,4)
   pval=pval/100
   res=write('skew.ctl','  'pval)
   n=n+1
endwhile
res=write('skew.ctl','tdef 1 linear 'time' 1hr')
res=write('skew.ctl','VARS 4')
res=write('skew.ctl','t       'zsize' 99  temperature')
res=write('skew.ctl','td      'zsize' 99  dewpoint')
res=write('skew.ctl','spd     'zsize' 99  dewpoint')
res=write('skew.ctl','dir     'zsize' 99  dewpoint')
res=write('skew.ctl','ENDVARS')
res=close('skew.ctl')
...
* Brice Coffer: StormSpd, StormDir based on umove/vmove
*   StormSpd = sqrt(pow(umove,2) + pow(vmove,2))
*   StormDir = 270.0 - (r2d * atan2(vmove2/umove2))
*   StormSpd=9.88
*   StormDir=267.47
*   StormSpd=19.7
*   StormDir=234.29
